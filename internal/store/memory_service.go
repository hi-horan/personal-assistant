...
func (s *Store) SaveMemory(ctx context.Context, rec MemoryRecord) (string, error) {
	...
	sourceSessionID, err := parseOptionalID(rec.SourceSessionID, "source_session_id")
	if err != nil {
		return "", err
	}
	sourceEventID, err := parseOptionalID(rec.SourceEventID, "source_event_id")
	if err != nil {
		return "", err
	}
	...
	err = tx.QueryRow(ctx, `
		...
		DO NOTHING
		RETURNING id
	`, memoryID, rec.AppName, rec.UserID, rec.Kind, rec.Content, metadataJSON, rec.Importance, sourceSessionID, sourceEventID, rec.CreatedAt).Scan(&memoryID)
	if err != nil {
	...
	s.logger.InfoContext(ctx, "memory saved", slog.String("memory_id", formatID(memoryID)), slog.String("app", rec.AppName), slog.String("user_id", rec.UserID), slog.String("kind", rec.Kind))
	return formatID(memoryID), nil
}
...
func (s *Store) SearchMemories(ctx context.Context, appName, userID, query string, limit int) ([]MemoryResult, error) {
	...
	rows, err := s.pool.Query(ctx, `
		WITH bm25_ranked AS (
			SELECT memory_id, bm25_score, rank
			FROM (
				SELECT
					c.memory_id,
					pdb.score(c.id) AS bm25_score,
					RANK() OVER (ORDER BY pdb.score(c.id) DESC) AS rank
		...
		vector_ranked AS (
			SELECT
				c.memory_id,
				GREATEST(0::double precision, 1 - (c.embedding <=> $4::vector)) AS vector_score,
				RANK() OVER (ORDER BY c.embedding <=> $4::vector) AS rank
			FROM memory_chunks c
			WHERE c.app_name = $1
			  AND c.user_id = $2
		...
		candidates AS (
			SELECT
				COALESCE(vector_ranked.memory_id, bm25_ranked.memory_id) AS memory_id,
				bm25_ranked.bm25_score,
				vector_ranked.vector_score,
				COALESCE(1.0 / (60 + bm25_ranked.rank), 0.0) +
				COALESCE(1.0 / (60 + vector_ranked.rank), 0.0) AS rrf_score
			FROM vector_ranked
			FULL OUTER JOIN bm25_ranked ON vector_ranked.memory_id = bm25_ranked.memory_id
		),
		memory_scores AS (
			SELECT
				memory_id,
		...
		SELECT
			...
			COALESCE(m.source_session_id, 0), COALESCE(m.source_event_id, 0),
			m.created_at,
			memory_scores.rrf_score +
			(0.10 * m.importance) AS score
		FROM memory_scores
		JOIN memories m ON m.id = memory_scores.memory_id
		ORDER BY score DESC, memory_scores.vector_score DESC, memory_scores.bm25_score DESC, m.updated_at DESC
		LIMIT $5
...
func (s *Store) recentMemories(ctx context.Context, appName, userID string, limit int) ([]MemoryResult, error) {
	rows, err := s.pool.Query(ctx, `
		SELECT
			...
			COALESCE(source_session_id, 0), COALESCE(source_event_id, 0),
...
func scanMemoryResults(rows pgx.Rows) ([]MemoryResult, error) {
	...
	for rows.Next() {
		...
		result.SourceSessionID = formatID(sourceSessionID)
		result.SourceEventID = formatID(sourceEventID)
